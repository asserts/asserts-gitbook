...
For example, the `kube_job_labels` metric from KSM usually has a `job_name` label to indicate the k8s job name. We could define a `Job` entity using `job_name` as its identifier, but `job_name` often ends with a GUID, so it may not be the ideal name. In this particular case, the customer has a label `label_heka_ai_task_name` that specifies the name of the framework task that creates all these k8s jobs. Using this task name instead of the `job_name` works better for them. Hence, they define the custom `Job` entity with the `label_heka_ai_task_name` label.&#x20;
